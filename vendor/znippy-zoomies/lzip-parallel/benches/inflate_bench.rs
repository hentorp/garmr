...
fn bench_inflate(c: &mut Criterion) {
    ...
    // Repetitive 4K
    {
        ...
        group.bench_function("repetitive_4k", |b| {
            b.iter(|| black_box(linflate::inflate_to_vec(&compressed, original.len()).unwrap()))
        });
    }
...
fn bench_flush_scan(c: &mut Criterion) {
    ...
    // 1MB with markers
    {
        ...
        group.bench_function("with_markers_1mb", |b| {
            b.iter(|| black_box(lzip_parallel::deflate_scan::find_all_flushes(&buf)))
        });
    }
...
fn bench_vs_miniz(c: &mut Criterion) {
    ...
    group.bench_function("lzip_64k", |b| {
        b.iter(|| black_box(linflate::inflate_to_vec(&compressed, original.len()).unwrap()))
    });
    ...
    group.bench_function("lzip_256k", |b| {
        b.iter(|| {
            black_box(linflate::inflate_to_vec(&compressed_256k, original_256k.len()).unwrap())
        })
    });
...
criterion_group!(benches, bench_inflate, bench_flush_scan, bench_vs_miniz);
