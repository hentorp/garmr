//! Streaming parallel JAR decompressor for large files.
//!
...
//!   parallel DEFLATE decode → yield JarEntry items.
//!
//! Only one batch of decompressed output is live at a time, bounding peak
//! memory regardless of JAR size.  The file is never fully loaded into RAM.
//!
...
/// Streaming parallel JAR decompressor.
///
...
/// Implements `Iterator<Item = Result<JarEntry, JarError>>`.
///
/// ```no_run
/// use ljar::reader::StreamingJarRead;
/// let f = std::fs::File::open("app.jar").unwrap();
/// for entry in StreamingJarRead::new(f).unwrap() {
///     let e = entry.unwrap();
///     println!("{}: {} bytes", e.name, e.data.len());
/// }
/// ```
pub struct StreamingJarRead<R: Read + Seek> {
    source: R,
    /// File entries sorted by local_header_offset for sequential disk reads.
    locations: Vec<crate::central_dir::EntryLocation>,
    next_entry: usize,
    buf: VecDeque<JarEntry>,
    error: bool,
}

impl<R: Read + Seek> StreamingJarRead<R> {
    ...
    pub fn new(mut source: R) -> Result<Self, JarError> {
    ...
    pub fn with_filter(mut source: R, needle: &str) -> Result<Self, JarError> {
        let all_locs = central_dir::read_central_directory_from(&mut source)?;
    ...
    fn fill_batch(&mut self) -> Result<(), JarError> {
        ...
        let mut raw: Vec<(String, Vec<u8>, u16, usize, u32)> = Vec::with_capacity(batch.len());
        for loc in batch {
            self.source
                .seek(SeekFrom::Start(loc.local_header_offset))
                .map_err(|_| JarError("seek to local header failed"))?;

            let mut hdr = [0u8; 30];
            self.source
                .read_exact(&mut hdr)
                .map_err(|_| JarError("read local header failed"))?;
            ...
            self.source
                ...
                .map_err(|_| JarError("seek past local header name/extra failed"))?;

            let mut compressed = vec![0u8; loc.compressed_size as usize];
            self.source
                .read_exact(&mut compressed)
                .map_err(|_| JarError("read compressed data failed"))?;
        ...
        // ── Parallel decode (LPT / heaviest-first) ─────────────────────────
        // Skewed archives (one fat entry among many tiny ones) tail-choke a plain
        // fan-out: the worker that grabs the monster runs alone while its siblings
        // idle. Weight each unit by its uncompressed size so the heavy entries are
        // claimed first and the cheap tail backfills → ~all cores stay busy.
        let results: Vec<Result<JarEntry, JarError>> =
            gatling::gatling_forkjoin::gatling_for_each_balanced(
                ...
                |i| raw[i].3 as u64,
                |i| {
                    let (name, compressed, method, expected_size, expected_crc) = &raw[i];
...
impl<R: Read + Seek> Iterator for StreamingJarRead<R> {
...
mod tests {
    ...
    fn streaming_single() {
        ...
        let entries: Vec<_> = StreamingJarRead::new(Cursor::new(&jar))
            .unwrap()
    ...
    fn streaming_matches_parallel() {
        ...
        let stream: Vec<_> = StreamingJarRead::new(Cursor::new(&jar))
            .unwrap()
            .collect::<Result<Vec<_>, _>>()
            .unwrap();
    ...
    /// the LPT (heaviest-first) balanced fan-out as the exact bytes that went in.
    /// The rebalance only changes the ORDER entries are claimed by workers, never
    /// the decoded payload or the file-offset output ordering.
    #[test]
    fn streaming_skewed_roundtrip_byte_identical() {
        // Deterministic, non-trivially-compressible fat entry so decode is real
        // work, plus a long tail of tiny entries (the skew).
        ...
        // Every input entry present with identical bytes (order-independent).
