...
//! ```
//!
//! JAR entries are **independent** (each its own DEFLATE/STORE stream, its own
...
//!
//! Usage: ljar <file.jar> [output-dir]   (no output-dir ⇒ list mode)
...
use ljar::central_dir::{EntryLocation, read_central_directory_from};
use ljar::chunk::decode_segment_into;
use ljar::entry::JarError;
...
fn local_data_offset(file: &mut File, loc: &EntryLocation) -> Result<u64, JarError> {
...
fn main() {
    ...
    if positional.is_empty() {
        eprintln!("usage: ljar [-n<workers>] <file.jar> [output-dir]");
        std::process::exit(1);
    }

    let jar_path = &positional[0];
    ...
    let mut file = File::open(jar_path).unwrap_or_else(|e| {
        eprintln!("error opening {jar_path}: {e}");
        std::process::exit(1);
    });
    ...
    let n_jobs = jobs.len();
    eprintln!("ljar: {n_jobs} entries, {n_workers} parallel writers");
    ...
    if stats.failed > 0 {
        eprintln!("ljar: {} extracted, {} failed", stats.ok, stats.failed);
    }
