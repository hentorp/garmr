...
ljar-rs uses the same parallel pipeline as lbzip2-rs, adapted for ZIP/DEFLATE.
...
## Rule 2 — Large JAR Entries: Chunked Parallel Decompress
...
ljar-rs follows the binary pattern: N workers, N segments, no oversplit.
...
## Rule 3 — Small JAR Entries: Single-Core
...
Zero-copy throughout is what ljar-rs is built for, same as lbzip2-rs. katana-osm
and Holger's artifact will use ljar-rs through the zero-copy pipeline — workers
decompress directly to pre-allocated output areas, writer thread receives data
with no intermediate copy.

The convenience APIs (`decompress_jar`, `decompress_jar_stream`) exist for testing
...
pub fn inflate_entry_into(compressed: &[u8], out: &mut Vec<u8>) -> Result<usize, JarError>;

// External — convenience, allocates
pub fn decompress_jar(data: &[u8]) -> Result<Vec<JarEntry>, JarError>;
pub fn decompress_jar_stream<R: Read + Seek>(src: R) -> Result<Vec<JarEntry>, JarError>;
pub struct StreamingJarRead<R: Read + Seek>;  // Iterator<Item = Result<JarEntry, JarError>>
```
...
Since each JAR entry's uncompressed size is known from the Central Directory
...
| Library API (`decompress_jar`, `decompress_jar_stream`) | ✅ done |
| Dedicated rayon pool (`ljar::thread_pool`)           | ✅ done |
...
| **Full pipeline binary (`src/bin/ljar.rs`)**         | ✅ done (v1) |
