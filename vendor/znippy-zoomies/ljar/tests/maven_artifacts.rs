//! Integration tests: extract 20 real Maven JARs and verify byte-perfect
//! output against the `zip` crate reference implementation.
//!
//! JARs are cached in `$LJAR_TEST_JARS` (default: `~/work/test-jars/`).
//! If missing, tests download from Maven Central on first run.
//!
//! Run:  cargo test --test maven_artifacts -- --nocapture
//! Skip: set `LJAR_SKIP_MAVEN_TESTS=1` to skip when offline.
...
/// Where to cache downloaded JARs.
fn jar_cache_dir() -> PathBuf {
    if let Ok(d) = std::env::var("LJAR_TEST_JARS") {
        return PathBuf::from(d);
    }
    dirs_or_home().join("work/test-jars")
}
...
/// Extract with ljar and build the same map.
fn ljar_entries(path: &Path) -> HashMap<String, Vec<u8>> {
    let file = fs::File::open(path).unwrap();
    let entries = ljar::decompress_jar_stream(file).unwrap();
    entries.into_iter().map(|e| (e.name, e.data)).collect()
}

/// Core verification: compare ljar output against zip crate reference.
/// Returns (entry_count, elapsed_secs) for timing.
fn verify_jar(jar: &TestJar) -> (usize, f64) {
    if std::env::var("LJAR_SKIP_MAVEN_TESTS").is_ok() {
        eprintln!("  SKIPPED (LJAR_SKIP_MAVEN_TESTS set)");
        return (0, 0.0);
    }
    ...
    let result = ljar_entries(&path);
    let elapsed = t0.elapsed().as_secs_f64();

    assert_eq!(
        reference.len(),
        result.len(),
        "{}: entry count mismatch (zip={}, ljar={})",
        jar.filename,
        reference.len(),
        result.len()
    );
...
/// Test the ljar binary end-to-end on a representative JAR.
#[test]
fn maven_cli_extract_guava() {
    ...
    let status = std::process::Command::new(env!("CARGO_BIN_EXE_ljar"))
        .arg(&jar_path)
        .arg(out_dir.path())
        .status()
        .expect("run ljar binary");
    assert!(status.success(), "ljar binary failed");
...
fn maven_timing_summary() {
    ...
    eprintln!(
        "  ljar Maven artifact timing — {} logical cores, {} physical cores",
        cores, physical
    );
    eprintln!(
        "═══════════════════════════════════════════════════════════════════════════════════════════════════════════════"
    ...
    eprintln!(
        "  {:40} {:>6} {:>8} {:>10} {:>9} {:>9} {:>9} {:>7}",
        "JAR", "Group", "Entries", "JAR size", "ljar", "unzip", "MB/s", "speedup"
    );
    eprintln!("  {}", "─".repeat(104));

    let mut total_entries = 0usize;
    let mut total_ljar_time = 0.0f64;
    ...
    for jar in TEST_JARS {
        ...
        let t0 = std::time::Instant::now();
        let result = ljar_entries(&path);
        let ljar_elapsed = t0.elapsed().as_secs_f64();
        ...
        let throughput = if ljar_elapsed > 0.0 {
            mb_out / ljar_elapsed
        } else {
            0.0
        };
        let speedup = if ljar_elapsed > 0.0 {
            unzip_elapsed / ljar_elapsed
        } else {
            0.0
        };
    ...
    eprintln!(
        ...
        total_ljar_time * 1000.0,
        total_unzip_time * 1000.0,
        if total_ljar_time > 0.0 {
            total_mb / total_ljar_time
        } else {
            0.0
        },
        total_speedup,
...
fn batch_timing() {
    ...
    // ── ljar batch (in-memory, no disk I/O) ──────────────────────────────────
    let (_, batch_stats) = ljar::batch::decompress_jars(&jar_paths).unwrap();
    let ljar_mb = batch_stats.decompressed_bytes_written as f64 / (1024.0 * 1024.0);

    eprintln!();
    eprintln!("  ljar batch (in-memory decompress):");
    eprintln!(
    ...
    // ── ljar batch (extract to disk) ─────────────────────────────────────────
    let out_dir = tempfile::tempdir().expect("create output dir");
    let t0 = std::time::Instant::now();
    let disk_stats = ljar::batch::extract_jars(&jar_paths, out_dir.path(), false).unwrap();
    let ljar_disk_elapsed = t0.elapsed().as_secs_f64();
    let ljar_disk_mb = disk_stats.decompressed_bytes_written as f64 / (1024.0 * 1024.0);

    eprintln!();
    eprintln!("  ljar batch (extract to disk):");
    eprintln!(
        "    {:.1} MB written in {:.1} ms",
        ljar_disk_mb,
        ljar_disk_elapsed * 1000.0
    );
    eprintln!(
        "    Throughput: {:.0} MB/s",
        if ljar_disk_elapsed > 0.0 {
            ljar_disk_mb / ljar_disk_elapsed
        } else {
            0.0
        }
    );
    ...
    let speedup_disk = if ljar_disk_elapsed > 0.0 {
        unzip_elapsed / ljar_disk_elapsed
    } else {
        0.0
    };
