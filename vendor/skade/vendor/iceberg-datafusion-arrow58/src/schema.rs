...
impl IcebergSchemaProvider {
    ...
    pub(crate) async fn try_new(
        ...
        // A table listed above can be dropped before we load it (skade's
        // compaction swap removes its scratch table concurrently with reader
        // sessions). Skip those instead of failing the whole provider — an
        // unrelated query must not 404 on a transiently-absent scratch table.
        let providers = try_join_all(
            table_names
                ...
                .map(|name| {
                    ...
                    async move {
                        match IcebergTableProvider::try_new(client, namespace, name).await {
                            Ok(p) => Ok(Some(p)),
