...
mod tests {
    ...
    /// A real, empty (no-snapshot) table loaded from a memory catalog. Unlike
    /// `get_test_table_from_metadata_file` (whose `TableMetadataV2Valid.json`
    /// fixture references manifest lists like `s3://a/b/2.avro` that do not
    /// exist on disk), this table is actually scannable: `IcebergTableScan::new`
    /// plans file tasks eagerly, so the static-provider scan tests need a table
    /// whose manifests can be read. The returned `TempDir` must be kept alive
    /// for the table's warehouse to remain valid.
    async fn get_test_scannable_static_table() -> (Table, TempDir) {
    ...
    async fn test_static_provider_scan() {
        let (table, _temp_dir) = get_test_scannable_static_table().await;
        let table_provider = IcebergStaticTableProvider::try_new_from_table(table.clone())
            .await
            .unwrap();
        let ctx = SessionContext::new();
