...
impl ParquetWriter {
    ...
    /// Build a [`DataFileBuilder`] for an **already-encoded, in-memory** Parquet
    /// file, deriving per-column statistics (`lower_bounds`/`upper_bounds`,
    /// `value_counts`, `null_value_counts`, `column_sizes`, `split_offsets`) from
    /// the file's own Parquet footer metadata.
    ///
    /// This is the exact same statistics path used by the streaming
    /// [`ParquetWriter`]/`DataFileWriter` close — it parses the footer of `bytes`
    /// and delegates to [`ParquetWriter::parquet_to_data_file_builder`] — so a
    /// hand-rolled writer (one that encodes Parquet itself and commits the
    /// resulting `DataFile`) gets byte-identical Iceberg column bounds and thus
    /// the same manifest-level data-file pruning.
    ///
    /// `schema` must be the table's current Iceberg schema (carrying field ids);
    /// `bytes` must be the complete Parquet file (it includes the footer);
    /// `file_path` is the final data-file location. `nan_value_counts` is left
    /// empty (matching `parquet_files_to_data_files`). The caller is responsible
    /// for setting `partition` / `partition_spec_id` on the returned builder.
    pub fn data_file_builder_from_parquet_bytes(
        schema: SchemaRef,
        bytes: &Bytes,
        file_path: String,
    ) -> Result<DataFileBuilder> {
        ...
        let metadata = parquet::file::metadata::ParquetMetaDataReader::new()
            ...
            .map_err(|err| {
                Error::new(
                    ErrorKind::DataInvalid,
                    format!("Error parsing in-memory Parquet metadata: {err}"),
                )
            })?;
        Self::parquet_to_data_file_builder(
            schema,
            Arc::new(metadata),
            written_size,
            file_path,
