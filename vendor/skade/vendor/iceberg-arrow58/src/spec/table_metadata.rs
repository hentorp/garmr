...
impl TableMetadata {
    ...
    /// Returns the table's snapshot references (branches and tags) keyed by ref
    /// name. The `main` branch, when present, is included. This is the whole
    /// `refs` map — the git-like surface (branch/tag enumeration) reads it.
    #[inline]
