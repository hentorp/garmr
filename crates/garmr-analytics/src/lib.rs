...
//! - Slice 2: [`risk`]-based alerting (RBA) — per-host (and per-staff) risk
//!   accumulated from adjudicated cases; a subject over threshold opens one
//!   risk case.
...
//! [`envdetect`], and source-[`silence`] detection — a known feed going dark
//! (Phase 14). Semantic (embedding) search shipped separately in the
//! `garmr-query`/`garmr-search` crates (Phase 6), not in this analytics crate.
